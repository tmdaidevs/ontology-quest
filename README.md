...
**Play it live:** https://tmdaidevs.github.io/ontology-quest/
...
| 3 | **Tools & Standards** | Match real-world technologies — RDF, RDFS, OWL, SKOS, SPARQL, JSON-LD, SHACL, Neo4j, Wikidata, schema.org, Protégé, Stardog, Amazon Neptune, Azure Cosmos DB Gremlin API, and GraphRAG — to their best-fit use cases in a matching game, then read short explainers for each. |
...
| 5 | **Build Your Own Ontology** | Design your own ontology in a sandbox node-link editor — add classes, instances, and labeled relationships on an interactive canvas, with full **undo/redo** as you build. Pick from 3 starter scenarios (Org Chart, Movie Recommendation System, Customer Support KB) or start fully custom, **export your graph as real Turtle (.ttl) or JSON-LD** syntax you can paste into any RDF tool, then validate against ontology best practices (has classes, has relationships, has a hierarchy, no orphan nodes, has instances) for a score. |
...
| Bonus | **Best Practices & Anti-Patterns** | Always unlocked. A technically valid ontology can still be badly designed. Expand four core modeling principles — Model the Domain Not the Database, Reuse Before You Rebuild, Open for Extension/Closed for Modification, and Prefer Composition Over Deep Hierarchies (illustrated with a live before/after diagram contrasting a brittle 6-level class hierarchy against a shallow, composable one) — each with a concrete before/after example, then browse a structural-guidance reference and a field guide of 8 common anti-patterns (System Silos, The Kitchen Sink, The God Object, The Time Machine, and more). Finish a 6-question scenario-diagnosis quiz to earn the Pattern Guardian badge. |
...
- **Progress rail** — the level-select map shows your 5 core levels as a connected node chain (done ✓ / active / locked), so your journey reads like the graphs you're learning about.
- **Badges** — per-level badges (History Scholar, Triple Builder, Perfect Matcher, Graph Navigator, Ontology Architect, Live Explorer, Six-Hop Voyager, Algorithm Adept, Enterprise Analyst, Semantic Analyst, Pattern Guardian) plus cross-level **meta-badges** for skilled play: Sharp Mind (no hints, 90+ score), Speedrunner (under a minute, 70+ score), Perfectionist (100% on every core level), and Completionist (finish all 10 levels).
- **Shareable score card** — generate a PNG snapshot of your score and badges to share anywhere.
...
- Client-side Turtle and JSON-LD serialization (Level 5 sandbox export) — hand-written, dependency-free graph-to-RDF-syntax conversion.
...
```
index.html                # Single-page app shell (landing, level map, 10 level/bonus screens)
...
js/
  ...
  ontology-export.js       # Graph → Turtle (.ttl) / JSON-LD serialization (Level 5 sandbox export)
  ...
  data/                    # Content data per level (timeline, concepts, tools, graphs, scenarios, algo-graph.js, case-studies.js, semantic-models.js, best-practices.js)
  levels/                  # One module per level (level1-history.js … level5-sandbox.js, level6-live.js, level7-algorithms.js, level8-enterprise.js, level9-semantic.js, level10-bestpractices.js bonus levels)
